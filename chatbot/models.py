...
class Messages(models.Model):
    ...
    def to_api_format(self):
        ...
        if self.role == 'assistant' and self.products_json:
            p_names = [p.get('name') for p in self.products_json if isinstance(p, dict) and p.get('name')]
            if p_names:
                text += f"\n\n[Displayed Product Cards: {', '.join(p_names)}]"
